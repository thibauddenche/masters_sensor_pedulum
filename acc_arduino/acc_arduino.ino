...
//  Serial.print(error, DEC);
  ...
  addValueToMobileAverageTab(dX);
  float value1 = calculMobileAverage();
  if (speed < angle * 8){
    speed = angle * 8;
    delay(1500);
  }
  else if (value1 < targetValueX - 0.02 && speed < 300) {
    speed += 2;
    delay(300);
  }
...
float calculMobileAverage() {
  ...
  return(tmp / (float)tabLen);
}

void addValueToMobileAverageTab(float value){
  if (value != 0.0) {
...
char circularBuffer[] = {0, 0, 0, 0, 0, 0, 0};
int buffLen = 7;
int indexx = 0;

void readAllValues() {
  while (Serial.available()){
    indexx = (indexx + 1) % buffLen;
    circularBuffer[indexx] = Serial.read();
  }
}

void getCircularBufferValues() {
 int goback = 0;
 int i = indexx;
 while (goback < 3) {
 if (i != 0)
 ...
 int j = 0;
 int count = 0;
 while (count < buffLen - 3 && !(circularBuffer[i] == 'a' && circularBuffer[(i + 1) % buffLen] == '|' && circularBuffer[(i + 2) % buffLen] == '|')) {
   if (i != 0)
     i = i - 1;
   else
     i = buffLen - 1;
 ...
 if (count < buffLen - 3) {
   i = (i + 3) % buffLen;
  if (angle != (int)circularBuffer[i]){
    angle = (int)circularBuffer[i];
    if (angle ==0)
      targetValueX = 0.0;
    else
...
void  serialWriteSpeed(int speed) {
  byte * b = (byte *) &speed;
  Serial.print("s||");
  Serial.write(b[0]);
  Serial.write(b[1]);
